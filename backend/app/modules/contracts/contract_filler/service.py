"""
Contract Filler Service

Business logic for the contract_filler tool module.
"""
...
def _require_owner(*, version: FilledVersion | None, current_user: User) -> FilledVersion:
    if not version:
...
def _version_to_public(version: FilledVersion) -> FilledVersionPublic:
    return FilledVersionPublic(
        id=version.id,
...
def update_version(
    ...
    *,
    current_user: User,
    version_id: uuid.UUID,
    version_in: FilledVersionUpdate,
) -> FilledVersionPublic:
    version = repository.get_version(session, version_id=version_id)
    _require_owner(version=version, current_user=current_user)
