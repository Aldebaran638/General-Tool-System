...
TEMPLATE_DIR = UPLOAD_ROOT / "contracts/contract_filler/template"
FILLED_DIR = UPLOAD_ROOT / "contracts/contract_filler/filled"
TEMPLATE_FILENAME = "工程承包合同20250607（Revised）.docx"
...
def get_template_path() -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError("Contract template file not found")
    return path
...
def save_filled_docx(*, source_path: Path, version_id: uuid.UUID, filename: str) -> tuple[str, int]:
    ...
    relative_path = f"contracts/contract_filler/filled/{stored_name}"
